...
public class ScriptMain : UserComponent
{
    ...
    public override void CreateNewOutputRows()
    {
        /*
           ...
           https://microsoft-ssis.blogspot.com/2017/07/active-directory-as-source.html
        */
        //string domainConnectionString = "LDAP://ou=users, ou=operations, DC=dcyf,DC=wa,DC=lcl";
        string domainConnectionString = "LDAP://OU=Servers,OU=Operations,DC=dcyf,DC=wa,DC=lcl";

        using (DirectorySearcher ds = new DirectorySearcher(new DirectoryEntry(domainConnectionString)))
        {
            StringBuilder sb = new StringBuilder("(&");
            sb.Append("(objectClass=computer)"); // Only computers and not groups
            sb.Append("(operatingSystem=Windows Server*)"); // All departments
            //sb.Append("(department=*)"); // All departments
            sb.Append(")");
            ...
            using (SearchResultCollection results = ds.FindAll())
            {
                ...
                foreach (SearchResult result in results)
                {
                    ...
                    Output0Buffer.displayname = GetPropertyValue(result, "displayname");
                    Output0Buffer.name = GetPropertyValue(result, "name");
                    Output0Buffer.operatingSystem = GetPropertyValue(result, "operatingSystem");
                    Output0Buffer.distinguishedname = GetPropertyValue(result, "distinguishedname");
                    Output0Buffer.description = GetPropertyValue(result, "description");
